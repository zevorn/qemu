...
Boot options
------------
The ``k230`` machine boots the K230 SDK through M-mode U-Boot. U-Boot reads the
Linux payload from an attached SD card image and then starts OpenSBI/Linux using
the normal SDK boot flow.
...
.. code-block:: bash
   ...
   $ qemu-system-riscv64 -machine k230,boot-both-cores=on -smp 2 \
      -bios "$SDK/little/uboot/u-boot" \
      -drive if=sd,file="$SDIMG",format=raw \
      -nographic

...
Attach an SDK SD card image with ``-drive if=sd``. The first SD drive is wired
to the SDK CANMV removable card slot, ``sdhci1@91581000``:
...
This flow starts SDK U-Boot in M-mode with ``-bios``. The Linux Image must be
rebuilt with standard RISC-V PTE bits before running under QEMU. You can either
boot from an attached SDK SD card image or place OpenSBI, Linux, initrd, and
DTB in RAM with loader devices and run ``bootm`` manually:
...
edits, and describe the initrd range in ``/chosen``. Replace
``${INITRD_END}`` with the host-calculated value above when typing the command.
``cma=0`` avoids the SDK kernel reserving too much of the little-core memory
window for initramfs boot.

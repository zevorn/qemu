...
/*
 * Winbond W25Q128JV, JESD216B Basic Flash Parameter Table only.
 * The layout follows the actual Winbond SFDP encoding (same family as
 * the w25q80bl/w25q512jv tables) and the field map used by Linux
 * spi-nor (sfdp.c):
 *   DWORD1  address bytes (3-byte only) and fast read capability bits
 *           (1-1-2, 1-2-2, 1-4-4, 1-1-4),
 *   DWORD2  density (128 Mbit - 1 -> 16 MiB),
 *   DWORD3  1-4-4 / 1-1-4 read settings (opcode, mode, wait clocks),
 *   DWORD4  1-1-2 / 1-2-2 read settings,
 *   DWORD8  erase types 1 (4 KiB, 20h) and 2 (32 KiB, 52h),
 *   DWORD9  erase type 3 (64 KiB, d8h),
 *   DWORD11 page size (256 bytes).
 * The table is padded to 256 bytes so the read mask stays a power of two.
 */
static const uint8_t sfdp_w25q128[] = {

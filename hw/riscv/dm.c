...
static void dm_execute_access_register(RISCVDMState *s, uint32_t command)
{
    ...
    if (transfer) {
        ...
        /*
         * Validate aarsize only when a known register type is targeted.
         * Unknown regno values are caught in the else branches below,
         * which directly set CMDERR_EXCEPTION without ROM execution.
         */
...
static uint64_t dm_haltsum0_post_read(RegisterInfo *reg, uint64_t val)
{
    RISCVDMState *s = RISCV_DM(reg->opaque);
    uint32_t result = 0;
    /* Align to 32-hart group boundary (HALTSUM0 reports 32 harts at a time) */
...
static bool dm_rom_realize(RISCVDMState *s, Error **errp)
{
    ...
    static const uint32_t rom_code_entry[] = {
        /* 0x800 <entry>: */
        0x0180006f,    /* j _entry:     0x818 - 0x800 = +0x18 */
        0x00000013,    /* nop                                  */

        /* 0x808 <resume>: */
        0x0600006f,    /* j _resume:    0x868 - 0x808 = +0x60 */
        0x00000013,    /* nop                                  */

        /* 0x810 <exception>: */
        0x0400006f,    /* j _exception: 0x850 - 0x810 = +0x40 */
        0x00000013,    /* nop                                  */
        ...
        /* andi s0, s0, RISCV_DM_MAX_HARTS - 1 */
        0x07f47413,
        DM_STORE(8, 0u, RISCV_DM_ROM_HARTID, 2u),
                       /* sw s0, HARTID(zero)                  */
        DM_LBU(8, 8, RISCV_DM_ROM_FLAGS),
                       /* lbu s0, FLAGS(s0)                    */
        /* andi s0, s0, RISCV_DM_FLAG_GOING */
        0x00147413,
        0x02041463,    /* bnez s0, going: 0x85c-0x838 = +0x24 */
        0xf1402473,    /* csrr s0, mhartid                     */
        /* andi s0, s0, RISCV_DM_MAX_HARTS - 1 */
        0x07f47413,
        DM_LBU(8, 8, RISCV_DM_ROM_FLAGS),
                       /* lbu s0, FLAGS(s0)                    */
        /* andi s0, s0, RISCV_DM_FLAG_RESUME */
        0x00247413,
        0xfc0410e3,    /* bnez s0, resume: 0x808-0x848 = -0x40*/
        0xfd5ff06f,    /* j entry_loop: 0x820-0x84c = -0x2c   */

        /* 0x850 <_exception>: */
        DM_STORE(0, 0u, RISCV_DM_ROM_EXCP, 2u),
                       /* sw zero, EXCP(zero)                  */
        0x7b202473,    /* csrr s0, dscratch0                   */
        0x00100073,    /* ebreak                               */

        /* 0x85c <going>: */
        DM_STORE(0, 0u, RISCV_DM_ROM_GOING, 2u),
                       /* sw zero, GOING(zero)                 */
        0x7b202473,    /* csrr s0, dscratch0                   */
        0xa9dff06f,    /* j whereto: 0x300-0x864 = -0x564      */

        /* 0x868 <_resume>: */
        0xf1402473,    /* csrr s0, mhartid                     */
        /* andi s0, s0, RISCV_DM_MAX_HARTS - 1 */
        0x07f47413,
        DM_STORE(8, 0u, RISCV_DM_ROM_RESUME, 2u),
                       /* sw s0, RESUME(zero)                  */
        0x7b202473,    /* csrr s0, dscratch0                   */
        0x7b200073,    /* dret                                 */
    };
...
static const VMStateDescription vmstate_riscv_dm = {
    ...
    .fields = (const VMStateField[]) {
        ...
        VMSTATE_VARRAY_UINT32(hart_resethaltreq, RISCVDMState, num_harts,
                              0, vmstate_info_bool, bool),
        VMSTATE_UINT32_ARRAY(hawindow, RISCVDMState,

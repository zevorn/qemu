...
static uint32_t sifive_plic_cpu_index_base(SiFivePLICState *plic)
{
    if (plic->cpu_index_base == SIFIVE_PLIC_CPU_INDEX_AUTO) {
        return plic->hartid_base;
    }

    return plic->cpu_index_base;
}
...
static void sifive_plic_realize(DeviceState *dev, Error **errp)
{
    ...
    for (i = 0; i < s->num_harts; i++) {
        RISCVCPU *cpu =
            RISCV_CPU(qemu_get_cpu(sifive_plic_cpu_index_base(s) + i));
    ...
    uint32_t aperture_size)
{
    ...
    cpu_base = sifive_plic_cpu_index_base(plic);

    for (i = 0; i < plic->num_addrs; i++) {
        int hartid = plic->addr_config[i].hartid;
        int cpu_num = cpu_base + hartid - hartid_base;
        CPUState *cpu = qemu_get_cpu(cpu_num);

        if (plic->addr_config[i].mode == PLICMode_M) {
            qdev_connect_gpio_out(dev, hartid - hartid_base + num_harts,
                                  qdev_get_gpio_in(DEVICE(cpu), IRQ_M_EXT));
        }
        if (plic->addr_config[i].mode == PLICMode_S) {
            qdev_connect_gpio_out(dev, hartid - hartid_base,

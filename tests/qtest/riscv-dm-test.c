...
/*
 * Read/write an XLEN-wide CSR via Access Register abstract command.
 * Automatically selects aarsize=3 on RV64 and aarsize=2 on RV32.
 */
static bool tcg_read_csr(QTestState *qts, uint32_t regno, uint64_t *val)
{
    if (is_rv64()) {
        return tcg_abstract_read_reg64(qts, regno, val);
    }
    uint32_t v32;
    if (!tcg_abstract_read_reg(qts, regno, &v32)) {
        return false;
    }
    *val = v32;
    return true;
...
static bool tcg_write_csr(QTestState *qts, uint32_t regno, uint64_t val)
{
    if (is_rv64()) {
        return tcg_abstract_write_reg64(qts, regno, val);
    }
    return tcg_abstract_write_reg(qts, regno, (uint32_t)val);
}
...
/*
 * Test: Unsupported command types (Quick Access, Access Memory)
 * should return CMDERR=2 (NOTSUP).
 */
static void test_unsupported_cmdtype(void)
{
    QTestState *qts = qtest_init("-machine virt");
    dm_set_active(qts);
    ...
    /* cmdtype=1 (Quick Access) should return CMDERR=2 (NOTSUP) */
    dm_write(qts, A_COMMAND, (1u << 24));
    uint32_t acs = dm_read(qts, A_ABSTRACTCS);
    uint32_t cmderr = (acs & ABSTRACTCS_CMDERR_MASK) >> ABSTRACTCS_CMDERR_SHIFT;
    g_assert_cmpuint(cmderr, ==, 2);

    /* Clear CMDERR */
    dm_write(qts, A_ABSTRACTCS, ABSTRACTCS_CMDERR_MASK);

    /* cmdtype=2 (Access Memory) should return CMDERR=2 (NOTSUP) */
    dm_write(qts, A_COMMAND, (2u << 24));
    acs = dm_read(qts, A_ABSTRACTCS);
    cmderr = (acs & ABSTRACTCS_CMDERR_MASK) >> ABSTRACTCS_CMDERR_SHIFT;
    g_assert_cmpuint(cmderr, ==, 2);
...
/*
 * Test: Commands are silently ignored when CMDERR is already set.
 */
static void test_command_ignored_when_cmderr(void)
{
    QTestState *qts = qtest_init("-machine virt");
    dm_set_active(qts);
    ...
    /* Trigger a cmderr via unsupported cmdtype */
    dm_write(qts, A_COMMAND, (1u << 24));
    uint32_t acs = dm_read(qts, A_ABSTRACTCS);
    uint32_t cmderr = (acs & ABSTRACTCS_CMDERR_MASK) >> ABSTRACTCS_CMDERR_SHIFT;
    g_assert_cmpuint(cmderr, !=, 0);

    /* Issue a valid command -- should be ignored while cmderr is set */
    uint32_t valid_cmd = (0u << 24) |   /* cmdtype = Access Register */
                         (1u << 17) |   /* transfer = 1 */
                         (2u << 20) |   /* aarsize = 32-bit */
                         0x1001;        /* regno = x1 */
    dm_write(qts, A_COMMAND, valid_cmd);

    /* cmderr should remain unchanged (not cleared, not overwritten) */
    uint32_t acs2 = dm_read(qts, A_ABSTRACTCS);
    g_assert_cmphex(acs2, ==, acs);

    qtest_quit(qts);
}

/*
 * Test: ABSTRACTAUTO PROGBUF0 triggers auto re-execution.
 */
static void test_abstractauto_progbuf(void)
{
    QTestState *qts = qtest_init("-machine virt");
    dm_set_active(qts);
    ...
    /* Issue a valid command to establish last_cmd */
    uint32_t cmd = (0u << 24) | (1u << 17) |
                   (2u << 20) | 0x1001;
    dm_write(qts, A_COMMAND, cmd);

    /* Ack: GOING then re-enter park loop */
    sim_cpu_going_ack(qts);
    sim_cpu_halt_ack(qts, 0);

    /* Enable autoexec on PROGBUF0 (bit 16) */
    dm_write(qts, A_ABSTRACTAUTO, (1u << 16));

    /* Write PROGBUF0 -> should trigger auto re-execution */
    dm_write(qts, A_PROGBUF0, 0x00100073);  /* ebreak */

    /* Command re-issued: BUSY should be set */
    uint32_t acs = dm_read(qts, A_ABSTRACTCS);
    g_assert_cmpuint(acs & ABSTRACTCS_BUSY, ==,
                     ABSTRACTCS_BUSY);

    /* Ack the auto-executed command */
    sim_cpu_going_ack(qts);
    sim_cpu_halt_ack(qts, 0);
...
static void test_tcg_halt_dpc(void)
{
    ...
    uint64_t dcsr;
    g_assert_true(tcg_read_csr(qts, REGNO_DCSR, &dcsr));
    ...
    uint64_t dpc;
    g_assert_true(tcg_read_csr(qts, REGNO_DPC, &dpc));
    ...
    uint64_t dpc2;
    g_assert_true(tcg_read_csr(qts, REGNO_DPC, &dpc2));
...
static void test_tcg_single_step(void)
{
    ...
    uint64_t dpc0;
    g_assert_true(tcg_read_csr(qts, REGNO_DPC, &dpc0));
    g_test_message("initial DPC = 0x%" PRIx64, dpc0);

    /* Step 3: Set DCSR.step = 1 */
    uint64_t dcsr;
    g_assert_true(tcg_read_csr(qts, REGNO_DCSR, &dcsr));
    g_test_message("DCSR before step = 0x%" PRIx64, dcsr);
    dcsr |= DCSR_STEP;
    g_assert_true(tcg_write_csr(qts, REGNO_DCSR, dcsr));

    /* Verify step bit is set */
    uint64_t dcsr_check;
    g_assert_true(tcg_read_csr(qts, REGNO_DCSR, &dcsr_check));
    ...
    g_assert_true(tcg_read_csr(qts, REGNO_DCSR, &dcsr));
    uint32_t cause = (dcsr & DCSR_CAUSE_MASK) >> DCSR_CAUSE_SHIFT;
    g_test_message("DCSR after step = 0x%" PRIx64 ", cause = %u", dcsr, cause);
    g_assert_cmpuint(cause, ==, DCSR_CAUSE_STEP);

    /* Step 6: Verify DPC advanced */
    uint64_t dpc1;
    g_assert_true(tcg_read_csr(qts, REGNO_DPC, &dpc1));
    g_test_message("DPC after step = 0x%" PRIx64, dpc1);
    g_assert_cmpuint(dpc1, !=, dpc0);

    /* Step 7: Step again to confirm repeatability */
    uint64_t dpc_prev = dpc1;
    g_assert_true(tcg_resume_hart_and_run(qts));
    g_assert_true(tcg_wait_for_halt(qts));
    ...
    uint64_t dpc2;
    g_assert_true(tcg_read_csr(qts, REGNO_DPC, &dpc2));
    g_test_message("DPC after 2nd step = 0x%" PRIx64, dpc2);
...
static void test_tcg_trigger_debug_mode(void)
{
    ...
    g_assert_true(tcg_read_csr(qts, REGNO_DCSR, &dcsr));
    uint32_t cause = (dcsr & DCSR_CAUSE_MASK) >> DCSR_CAUSE_SHIFT;
    g_test_message("DCSR after trigger = 0x%" PRIx64
                   ", cause = %u", dcsr, cause);
...
/*
 * Test: HAWINDOW multi-hart halt and resume.
 * Select harts 0,1,2 via HAWINDOW, halt all via HASEL, verify haltsum0,
 * then resume all via HASEL and verify resumeack.
 */
static void test_hawindow_multi_halt_resume(void)
{
    QTestState *qts = qtest_init("-machine virt -smp 64");
    dm_set_active(qts);

    /* Select harts 0, 1, 2 via HAWINDOW[0] */
    dm_write(qts, A_HAWINDOWSEL, 0);
    dm_write(qts, A_HAWINDOW, 0x7);  /* bits 0,1,2 set */

    /* Enable HASEL and issue haltreq */
    dm_write(qts, A_DMCONTROL,
             DMCONTROL_DMACTIVE | DMCONTROL_HASEL | DMCONTROL_HALTREQ);

    /* Simulate halt ack for harts 0, 1, 2 */
    sim_cpu_halt_ack(qts, 0);
    sim_cpu_halt_ack(qts, 1);
    sim_cpu_halt_ack(qts, 2);

    /* Verify all three halted via haltsum0 (hartsel=0 selects group 0) */
    dm_write(qts, A_DMCONTROL, DMCONTROL_DMACTIVE | DMCONTROL_HASEL);
    uint32_t haltsum0 = dm_read(qts, A_HALTSUM0);
    g_assert_cmphex(haltsum0 & 0x7, ==, 0x7);  /* harts 0,1,2 halted */
    g_assert_cmphex(haltsum0 & 0x8, ==, 0);     /* hart 3 not halted */

    /* Issue resumereq via HASEL to resume all selected harts */
    dm_write(qts, A_DMCONTROL,
             DMCONTROL_DMACTIVE | DMCONTROL_HASEL | DMCONTROL_RESUMEREQ);

    /* Simulate resume ack for harts 0, 1, 2 */
    sim_cpu_resume_ack(qts, 0);
    sim_cpu_resume_ack(qts, 1);
    sim_cpu_resume_ack(qts, 2);

    /* Verify haltsum0: all three harts should no longer be halted */
    dm_write(qts, A_DMCONTROL, DMCONTROL_DMACTIVE);
    haltsum0 = dm_read(qts, A_HALTSUM0);
    g_assert_cmphex(haltsum0 & 0x7, ==, 0);

    /* Verify resumeack per-hart by selecting each one individually */
    uint32_t dmstatus;
    for (int h = 0; h < 3; h++) {
        uint32_t sel = DMCONTROL_DMACTIVE | (h << 6);
        dm_write(qts, A_DMCONTROL, sel);
        dmstatus = dm_read(qts, A_DMSTATUS);
        g_assert_cmpuint(dmstatus & DMSTATUS_ALLRESUMEACK, !=, 0);
    }

    qtest_quit(qts);
}

static QTestState *sba_dm_init(void)
{
    QTestState *qts = qtest_init(
        "-machine virt"
        " -global riscv-dm.sba-addr-width=32");
    dm_set_active(qts);
    return qts;
}

/*
 * Test: SBA sticky error and W1C clear.
 */
static void test_sba_sticky_error(void)
{
    QTestState *qts = sba_dm_init();

    uint32_t sbcs = dm_read(qts, A_SBCS);
    uint32_t sbasize =
        (sbcs & SBCS_SBASIZE_MASK) >> SBCS_SBASIZE_SHIFT;
    if (sbasize == 0) {
        g_test_skip("SBA not enabled");
        qtest_quit(qts);
        return;
    }

    /* Configure 32-bit access + read-on-address */
    sbcs = SBCS_SBACCESS32 | (1u << 20);
    dm_write(qts, A_SBCS, sbcs);

    /* Try unaligned address to trigger error */
    dm_write(qts, A_SBADDRESS0, 0x3);
    sbcs = dm_read(qts, A_SBCS);
    uint32_t sberror = (sbcs & SBCS_SBERROR_MASK) >> 12;

    if (sberror == 0) {
        /* Try unmapped address */
        dm_write(qts, A_SBADDRESS0, 0xDEAD0000);
        sbcs = dm_read(qts, A_SBCS);
        sberror = (sbcs & SBCS_SBERROR_MASK) >> 12;
    }

    if (sberror != 0) {
        /* Sticky: another SBA op keeps same error */
        dm_write(qts, A_SBADDRESS0, 0x80000000);
        uint32_t sbcs2 = dm_read(qts, A_SBCS);
        uint32_t sberror2 = (sbcs2 & SBCS_SBERROR_MASK) >> 12;
        g_assert_cmpuint(sberror2, ==, sberror);

        /* W1C clear */
        dm_write(qts, A_SBCS, sberror << 12);
        uint32_t sbcs3 = dm_read(qts, A_SBCS);
        uint32_t sberror3 = (sbcs3 & SBCS_SBERROR_MASK) >> 12;
        g_assert_cmpuint(sberror3, ==, 0);
    } else {
        g_test_skip("Could not trigger SBA error");
    }
...
int main(int argc, char *argv[])
{
    ...
    qtest_add_func("/riscv-dm/hawindow-multi-halt-resume",
                   test_hawindow_multi_halt_resume);
    qtest_add_func("/riscv-dm/sba-sticky-error",
                   test_sba_sticky_error);
    ...
    qtest_add_func("/riscv-dm/unsupported-cmdtype",
                   test_unsupported_cmdtype);
    qtest_add_func("/riscv-dm/command-ignored-when-cmderr",
                   test_command_ignored_when_cmderr);
    qtest_add_func("/riscv-dm/abstractauto-progbuf",
                   test_abstractauto_progbuf);
